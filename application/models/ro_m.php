...
class ro_m extends CI_Model {
    ...
    public function get($tahun, $bulan)
    {
        $query = 
            "SELECT 
                    CONCAT(nama_bulan,' ',TAHUN) as periode,
                    a.*, 
                    total* harga as total_harga 
            from 
                (SELECT TB_MPP.TAHUN,(SELECT nama_supplier from data_supplier where kode_supplier = data_cm.supplier) as nama_sup,
                    data_CM.part_number,
                    data_cm.harga,
                    data_cm.id_cm,
                    part_name, 
                    nama_bulan,
                    sum(FLOOR(bom.kebutuhan/data_cm.komposisi*tb_mpp.jumlah)) as total FROM bom, 
                    data_cm, 
                    tb_mpp, 
                    tb_bulan 
            where data_cm.id_cm = bom.id_cm and tb_bulan.id_bulan = tb_mpp.id_bulan and tb_mpp.tahun = '$tahun' and tb_mpp.id_bulan = '$bulan' and tb_mpp.id_mobil = bom.id_mobil GROUP BY part_name, nama_bulan) a ";
        return $this->db->query($query);
